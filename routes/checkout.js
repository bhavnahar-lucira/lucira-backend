...
// findOneAndUpdate result across driver shapes ({value: doc} vs doc) — same
// guard routes/recommendations.js uses. Reading the wrong shape here would look
// like a record with no snapshot and park a PAID checkout as FAILED.
const unwrapFindOneAndUpdate = (result) => (result && result.value !== undefined ? result.value : result);
...
async function finalizeRazorpayCheckout(
  ...
  const claimed = unwrapFindOneAndUpdate(await coll.findOneAndUpdate(
  ...
  if (!claimed) {
    ...
    if (source === "browser" && requestBody) {
      ...
      const claim2 = unwrapFindOneAndUpdate(await coll.findOneAndUpdate(
        { _id: razorpayOrderId, status: { $in: ["PENDING", "FAILED"] }, attempts: { $lt: FINALIZE_MAX_ATTEMPTS } },
        { $set: { status: "PROCESSING", razorpayPaymentId, source, updatedAt: new Date() }, $inc: { attempts: 1 } },
        { returnDocument: "after" }
      ));
...
async function routes(fastify, options) {
      ...
      if (shouldPreferBodyItems) {
        ...
        const itemsToSave = bodyItems.map(incomingItem => {
          ...
          return {
            ...
            // Carried through so repriceItems can hold an in-stock line to the
            // single unit the storefront lets the shopper buy — dropping it here
            // let this path bill a quantity the cart never showed.
            inStock: incomingItem.inStock,
            price: resolvedPrice,
  ...
  // The browser's report that the Razorpay sheet succeeded. It no longer *does*
  // the work — it runs the same idempotent finalizer the webhook and the
  // reconciler run, against the razorpay_checkouts record written at /order
  // time. Whichever caller arrives first creates the Shopify order; the others
  // read the completed record. So closing the tab here costs nothing.
  fastify.post('/payment/razorpay/complete', async (request, reply) => {
    try {
      ...
      try {
        // `requestBody` is the fallback for a checkout that predates the record
        // (or whose /order-time write failed) — the browser is the only caller
        // that still holds the full cart context.
        const result = await finalizeRazorpayCheckout(db, {
          razorpayOrderId,
          razorpayPaymentId,
          source: "browser",
          requestBody: body,
        ...
        if (err instanceof FinalizeStateError) {
          if (err.code === "IN_PROGRESS") {
            ...
            const rec = await db.collection(RAZORPAY_CHECKOUTS_COLLECTION).findOne({ _id: razorpayOrderId });
          ...
          if (err.code === "EMPTY_CART" || err.code === "NO_RECORD") {
