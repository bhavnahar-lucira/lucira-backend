...
// The storefront sells an in-stock piece one at a time: it's the single item
// actually on hand, so the cart locks that line to quantity 1 and hides the
// selector (CartItem's canEditQuantity, cartSlice's computedQuantity). That
// rule used to live only in the browser, so a stored line carrying a higher
// quantity — the same piece added twice, or one that was made-to-order when it
// went into the cart and has since been restocked — still priced at that
// quantity here. The cart showed one unit, the draft order billed two, and
// checkout's total check then refused the payment on every retry.
//
// `inStock` is the same flag the storefront clamps on (cart.js persists it per
// line), so both sides read one value and can't drift apart. Insurance is a
// flat add-on the storefront exempts, and free gifts never reach here — the
// offer rules own their quantity.
function lineQuantity(item = {}) {
  const stored = Math.max(1, Number(item.quantity || 1));
  const inStock = item.inStock === true || item.inStock === "true";
  const isInsurance = normalizeVariantId(item.variantId) === INSURANCE_VARIANT_ID;
  return inStock && !isInsurance ? 1 : stored;
}
