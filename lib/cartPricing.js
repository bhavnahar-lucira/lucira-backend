...
// Array of authorized free gift variant IDs (excluding gold coins)
// To change the free gift in the future, update the .env file or modify this array.
const AUTHORIZED_FREE_GIFTS = [
  process.env.FREE_GIFT_VARIANT_ID || "gid://shopify/ProductVariant/48414958715098", // Default: Silver Diamond Bracelet
];
// Exporting the first one as SILVER_BRACELET_VARIANT_ID for backward compatibility with settings API
const SILVER_BRACELET_VARIANT_ID = AUTHORIZED_FREE_GIFTS[0];
...
function isFreeGiftVariant(variantId = "") {
  return isGoldCoinVariant(variantId) || AUTHORIZED_FREE_GIFTS.includes(variantId);
}
...
async function repriceItems(items = [], { dropInvalid = false } = {}) {
  const rawSource = Array.isArray(items) ? items : [];
  if (!rawSource.length) {
    return { items: [], changed: false, diamondTotal: 0, removed: [] };
  }

  // Collapse duplicate lines for the same variant into one, summing quantity.
  // /api/cart/add's read-then-write isn't atomic and nothing enforces one
  // line per variant, so two near-simultaneous adds of a not-yet-in-cart
  // item can each decide "not here yet" and both push a line. This is the
  // one place both the cart and the checkout re-price from, so fixing it
  // here self-heals any cart that already picked up a duplicate, not just
  // ones created after this change. Merge key matches the one /add already
  // uses (variantId alone) so this never combines lines /add would keep
  // apart.
  ...
  for (const item of rawSource) {
    ...
    const existingIndex = vid ? indexByVariant.get(vid) : undefined;
    if (existingIndex !== undefined) {
      source[existingIndex] = {
        ...source[existingIndex],
        quantity: Number(source[existingIndex].quantity || 0) + Number(item.quantity || 0),
      };
      mergedDuplicates = true;
    } else {
      if (vid) indexByVariant.set(vid, source.length);
      source.push(item);
    }
  }
