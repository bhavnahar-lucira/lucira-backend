...
// ---------------------------------------------------------------------------
// One collection's share of the store-wide pass.
//
// The 6-hour cap above only helps if the loop keeps turning. On 22 Sep 2026 it
// did not: the pass reached 130 of 1,055 collections in seven minutes and then
// sat on one collection for fifteen hours, never reaching the cap check,
// leaving no error and no heartbeat. A stalled GA4 request was the culprit and
// is now bounded in lib/ga4.js — but the pass must not depend on every
// dependency being well-behaved, so each collection gets its own deadline.
//
// A blown deadline counts the collection as failed and moves on; the next pass
// picks it up. The underlying promise cannot be cancelled and may still settle
// later, which is harmless — its result is simply ignored.
//
// Sized well above reality: a collection normally takes a second or two, and
// 130 went through in seven minutes on the run that later wedged.
const COLLECTION_DEADLINE_MS = 5 * 60 * 1000;
...
function withDeadline(promise, ms, label) {
  ...
  return Promise.race([
    ...
    new Promise((_, reject) => {
      timer = setTimeout(
        () => reject(new Error(`${label}: gave up after ${Math.round(ms / 1000)}s — skipped, next pass will retry`)),
        ms
      );
    })
...
// ---------------------------------------------------------------------------
// Sync scheduling vocabulary now lives in lib/syncCadence.js, shared with the
// from-same-collection scheduler. Re-exported below so existing callers keep
// working unchanged.
//
// One meaning that IS specific to this module: 'manual' is the one-time sort.
// Only "Sync now" pushes an order, so the collection is ordered once and then
// stays frozen — and the rule doc still EXISTS, which is the point, because
// runGlobalRule skips every collection that has a rule of its own. A one-time
// sort is therefore not quietly overwritten by the nightly store-wide pass.
// ---------------------------------------------------------------------------
const {
  SYNC_MODES, DEFAULT_SYNC_WEEKDAY, syncModeOf, syncWeekdayOf
} = require('./syncCadence');
...
/**
 * The live order, and the sortOrder mode, straight from Shopify.
 *
 * `priority` matters here in a way it does not for the other scans: this
 * snapshot is the LEFT side of planMoves, so everything between reading it and
 * writing the moves is a window in which the collection can drift. The
 * governor's background lane pauses while the shop's cost bucket is low, which
 * would widen that window. A manual "Sync now" therefore reads at interactive
 * priority (a person is waiting and the write follows immediately); the
 * nightly and global passes stay in the background lane, where yielding to
 * shopper traffic is worth more than a narrower window.
 */
async function fetchLiveOrder(collectionId, priority = 'background') {
  const ids = [];
  let after = null;
  let sortOrder = null;
  let pages = 0;
  do {
    const data = await shopifyAdminFetch(ORDER_SCAN_QUERY, { id: collectionId, first: 250, after }, { priority });
...
async function buildSmartContext(fastify, rule) {
  const [products, popMaps, metrics, cardPriceOf] = await Promise.all([
    ...
    // The handle matters: the card prices a 9KT variant in 9kt-collection.
    // For the global rule this is the per-collection copy, so it is the real
    // collection's handle, not __all_collections__.
    loadCardPriceResolver(rule, { collectionHandle: rule.collectionHandle })
  ]);
...
// ---------------------------------------------------------------------------
// Collection listing and search now live in lib/collectionSearch.js, shared
// with the from-same-collection module. Re-exported below so existing callers
// (and the route layer) keep working unchanged.
// ---------------------------------------------------------------------------
const {
  listAllCollections,
...
// ---------------------------------------------------------------------------
async function pushOrderForCollection(fastify, rule, { onWarn, priority = 'background' } = {}) {
  const [{ ordered }, liveOrder] = await Promise.all([
    computeOrderForRule(fastify, rule),
    fetchLiveOrder(rule.collectionId, priority)
  ]);
  ...
  for (let i = 0; i < moves.length; i += MOVE_BATCH_SIZE) {
    ...
    //
    // The LAST chunk is waited on too. collectionReorderProducts is
    // asynchronous: it returns a job id and Shopify applies the moves after
    // the mutation responds. Returning without waiting made the sync lie about
    // being finished — observed live on 04 Sep, when two syncs 12s apart each
    // reported 36 moves (the second re-planned against an order the first
    // job had not finished writing), and a preview taken straight after a sync
    // showed 43 products "will move" when the true answer was 0. It converges
    // either way, but only after a delay nobody can see.
    if (jobId) {
      const done = await waitForJob(jobId);
      if (!done && onWarn) {
        onWarn(i + MOVE_BATCH_SIZE < moves.length
          ? 'Reorder job did not report completion in time; later chunks may have applied out of order'
          : 'Reorder job did not report completion in time; the new order may take a moment to appear');
      }
    }
  }
  return out;
...
async function previewSmartRule(fastify, rule) {
  ...
  const products = ordered.map((p, newIndex) => {
    ...
    const card = ctx.cardPriceOf ? ctx.cardPriceOf(p) : null;
    return {
      id: p.id,
      title: p.title,
      handle: p.handle,
      image: p.image,
      // Lowest variant price (what `price` means in the engine)...
      price: p.price,
      compareAtPrice: p.compareAtPrice || 0,
      // ...and the variant the storefront card actually shows. The tile leads
      // with this one, or the preview hides exactly the mismatch it exists to
      // catch. null when the in-stock scan was unavailable.
      cardPrice: card ? card.price : null,
      cardCompareAtPrice: card ? card.compareAtPrice : null,
      cardVariant: card ? card.variantTitle : null,
      cardPriceSource: card ? card.source : null,
...
async function runSmartRule(fastify, rule, trigger, opts = {}) {
  ...
  try {
    ...
    try {
      // A person clicked Sync now — read the live order in the interactive
      // lane so the snapshot cannot pause between read and write.
      const result = await pushOrderForCollection(fastify, rule, {
        onWarn: pushError,
        priority: trigger === 'manual' ? 'interactive' : 'background'
      });
...
async function getProductInsights(fastify, productId) {
  ...
  // Same windows the rules offer, so Product Information can answer "why did
  // this land in the 90-day slot" with the number the slot actually used.
  const windows = ['d3', 'd7', 'd30', 'd90'].map((w, i) => {
