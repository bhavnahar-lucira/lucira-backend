...
const { scheduleCacheClear, queueRevalidation } = require('./storefrontRevalidation');
...
async function runSmartRule(fastify, rule, trigger, opts = {}) {
  ...
  try {
    ...
    try {
      ...
      // The new order is in Shopify, but this server's collection cache and the
      // cached /collections/<handle> page still hold the old one.
      if (stats.moves > 0 || stats.sortOrderChanged) {
        scheduleCacheClear(`smart-sort:${rule.collectionHandle}`);
        queueRevalidation({ collections: [rule.collectionHandle] }, `smart-sort:${rule.collectionHandle}`,
          { quietMs: 2000 });
      }
      return { runId, ...stats };
    } catch (err) {
    ...
    try {
      ...
      for (const c of targets) {
        ...
        try {
          ...
          if (r.moves > 0 || r.sortOrderChanged) changedHandles.push(c.handle);
      ...
      // Cleared once at the end rather than per collection: the pass runs for
      // hours, and wiping the cache every few seconds would leave it cold.
      if (changedHandles.length) {
        scheduleCacheClear('smart-sort:global');
        queueRevalidation({ collections: changedHandles }, 'smart-sort:global');
      }
      return { runId, ...stats };
    } catch (err) {
