...
// ---------------------------------------------------------------------------
// Sync scheduling vocabulary. Shared by the routes (validation + defaults) and
// the scheduler (the due check) so "what does this rule's schedule mean" has
// exactly one answer:
//
//   daily   - every day at scheduleTime (IST). The original behaviour.
//   weekly  - once a week, on syncWeekday at scheduleTime (IST).
//   manual  - never on a schedule. Only "Sync now" pushes an order, so the
//             collection is sorted once and then stays frozen. The rule doc
//             still EXISTS, and that is the point: runGlobalRule skips every
//             collection that has a rule of its own, so a one-time sort is not
//             quietly overwritten by the nightly store-wide pass.
//
