...
// Customer entitlement only. Every other field is deliberately left out so
// this can run against a live rule (the occasion coupons — see
// lib/occasionCoupons.js) without resending, and possibly stomping, the value,
// dates or product scope staff last saved.
const UPDATE_CODE_CUSTOMERS_MUTATION = `
  mutation discountCodeCustomers($id: ID!, $basicCodeDiscount: DiscountCodeBasicInput!) {
    discountCodeBasicUpdate(id: $id, basicCodeDiscount: $basicCodeDiscount) {
      codeDiscountNode { id }
      userErrors { field code message }
    }
...
const DEACTIVATE_CODE_MUTATION = `
  mutation discountCodeDeactivate($id: ID!) {
    discountCodeDeactivate(id: $id) {
      codeDiscountNode { id codeDiscount { ... on DiscountCodeBasic { status startsAt endsAt } } }
      userErrors { field code message }
    }
  }
`;
...
const DEACTIVATE_AUTO_MUTATION = `
  mutation discountAutomaticDeactivate($id: ID!) {
    discountAutomaticDeactivate(id: $id) {
      automaticDiscountNode { id automaticDiscount { ... on DiscountAutomaticBasic { status startsAt endsAt } } }
      userErrors { field code message }
    }
  }
`;
...
function buildCodeDiscountInput(rule, previousRule) {
  ...
  input.combinesWith = {
    orderDiscounts: Boolean(rule.combineCoupons),
    productDiscounts: Boolean(rule.combineCoupons),
    shippingDiscounts: Boolean(rule.combineCoupons),
  };
...
/**
 * Add/remove customers on a code discount's "specific customers" selection.
 *
 * Shopify's customer selection is delta-based like its items input, so only
 * the difference is ever sent. Chunked because the add list is a whole day's
 * worth of birthdays — a few hundred ids in one mutation is a rejection
 * waiting to happen.
 */
async function updateDiscountCustomers(shopifyDiscountId, { add = [], remove = [] }) {
  const CHUNK = 100;
  const chunks = [];
  for (let i = 0; i < add.length; i += CHUNK) chunks.push({ add: add.slice(i, i + CHUNK), remove: [] });
  for (let i = 0; i < remove.length; i += CHUNK) chunks.push({ add: [], remove: remove.slice(i, i + CHUNK) });
  if (!chunks.length) return { added: 0, removed: 0 };

  for (const customers of chunks) {
    const data = await shopifyAdminFetch(
      UPDATE_CODE_CUSTOMERS_MUTATION,
      { id: shopifyDiscountId, basicCodeDiscount: { customerSelection: { customers } } },
      { priority: 'background' }
    );
    throwOnUserErrors(data.discountCodeBasicUpdate, 'discountCodeBasicUpdate (customers)');
  }

  return { added: add.length, removed: remove.length };
}

// Deactivate/activate don't toggle a separate "paused" flag — Shopify
// implements them by moving endsAt (deactivate sets it to "now", activate
// clears it back to null). Returning startsAt/endsAt here (not just status)
// lets the caller persist that change locally too — otherwise the local
// record keeps whatever endsAt a previous sync/deactivate captured, and the
// next "Save & Sync" resends that now-stale, now-in-the-past endsAt to
// Shopify and silently re-expires a discount the staff member just reactivated.
async function deactivateShopifyDiscount(shopifyDiscountId, method) {
  ...
  const d = isCode ? node?.codeDiscount : node?.automaticDiscount;
  return { status: d?.status, startsAt: d?.startsAt, endsAt: d?.endsAt };
}
