...
const governor = require('./shopifyGovernor');
...
// Shopify reports a drained cost bucket as HTTP **200** with a `Throttled`
// entry in the GraphQL errors array — fetchWithRetry only sees 429/5xx, so it
// cannot help here. These retries are governed by the server's own
// throttleStatus (see lib/shopifyGovernor.js), not by a fixed sleep.
const THROTTLE_MAX_ATTEMPTS = Number(process.env.SHOPIFY_THROTTLE_ATTEMPTS) || 6;

const isThrottleError = (errors) => Array.isArray(errors) && errors.some((e) =>
  e?.extensions?.code === 'THROTTLED' || /throttl/i.test(e?.message || '')
);

/**
 * Admin GraphQL call, queued through the cost governor.
 *
 * @param {string} query
 * @param {object} [variables]
 * @param {object} [options]
 * @param {'interactive'|'background'} [options.priority='interactive']
 *        `background` yields to shopper traffic and pauses while the shop's
 *        cost bucket is low. Use it for scans, schedulers and warm-ups —
 *        anything no one is waiting on.
 * @param {string} [options.apiVersion='2024-10']
 */
async function shopifyAdminFetch(query, variables = {}, options = {}) {
  ...
  // `priority` and `apiVersion` are ours; everything else is passed to fetch.
  // They must not reach RequestInit — `priority` collides with fetch's own
  // 'high'|'low'|'auto' option and would throw.
  const { priority, apiVersion, ...fetchOptions } = options;
  const url = 'https://' + SHOP_DOMAIN + '/admin/api/' + (apiVersion || '2024-10') + '/graphql.json';
  const label = governor.operationName(query) || 'anonymous';

  return governor.schedule(query, async () => {
    for (let attempt = 0; attempt < THROTTLE_MAX_ATTEMPTS; attempt++) {
      ...
      // Shop-wide truth, even on a throttled response — and especially then.
      governor.sync(cost);
      governor.learn(query, cost);

      if (data.errors) {
        if (isThrottleError(data.errors)) {
          const waitMs = governor.throttleBackoffMs(cost, attempt);
          console.warn(
            `[Shopify] ${label} throttled (attempt ${attempt + 1}/${THROTTLE_MAX_ATTEMPTS}) — ` +
            `bucket ${cost?.throttleStatus?.currentlyAvailable ?? '?'}/${cost?.throttleStatus?.maximumAvailable ?? '?'}, ` +
            `needs ${cost?.requestedQueryCost ?? '?'}; waiting ${waitMs}ms`
          );
          await governor.sleep(waitMs);
          continue;
        }
    ...
    throw new Error(
      `Shopify Admin API still throttled after ${THROTTLE_MAX_ATTEMPTS} attempts (${label}). ` +
      'The shop cost bucket is saturated — check for a competing app or a runaway scan.'
    );
  }, { priority });
}
...
module.exports = {
  ...
  waitForFileReady,
  getShopPricingData,
  // Live bucket level, queue depths, learned per-operation costs. Surfaced on
  // /health so a throttle can be diagnosed without adding logging.
