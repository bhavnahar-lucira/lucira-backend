...
// The trailing windows every metric is reported over. THE single source of
// truth — the order scan, the GA4 pulls, the fallback maps and the `dN` keys
// are all derived from this list, so adding a window is a one-line change here
// plus the matching ATTRIBUTES/SORT_KEYS entries in recommendations.js.
//
// 90 days was added 22 Sep 2026 because 30 was too short to fill slots on a
// catalogue this size. Measured on this property the same day: products with
// any add-to-cart signal go from 3,155 (30d) to 5,904 (90d), and with any view
// from 23,440 to 52,397. GA4 holds the history (data on every one of the last
// 120 days) and Shopify holds the orders (271 in 30d, 761 in 90d). The
// first-party beacon only starts 1 Sep 2026, so a 90d window falls back to
// something much thinner if GA4 is ever switched off — which is what
// `sources` and `viewsTrackingSince` exist to disclose.
const WINDOWS = [3, 7, 30, 90];
...
async function shopifyOrderWindows() {
  // The cache key carries the span: widening the window must not be served a
  // narrower cached result left over from the previous deploy.
  const maxDays = Math.max(...WINDOWS);
  return getServerCache(`reco-signals:orders-${maxDays}d`, async () => {
    const sinceIso = new Date(Date.now() - maxDays * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    // Every window but the widest needs its own cutoff; the widest is the scan.
    const cuts = WINDOWS.map((d) => ({ key: 'd' + d, cut: Date.now() - d * 24 * 60 * 60 * 1000 }));

    const orders = Object.fromEntries(WINDOWS.map((d) => ['d' + d, new Map()]));
    const revenue = Object.fromEntries(WINDOWS.map((d) => ['d' + d, new Map()]));
    ...
    do {
      ...
      for (const order of page.nodes || []) {
        ...
        for (const li of order.lineItems?.nodes || []) {
          ...
          const fresh = !perOrderSeen.has(pid);
          if (fresh) perOrderSeen.add(pid);
          for (const { key, cut } of cuts) {
            if (t < cut) continue;
            bump(revenue[key], pid, amount);
            if (fresh) bump(orders[key], pid, 1); // an order counts once per product
          }
        }
      }
      ...
      if (after && pages >= 40) after = null; // 10k orders in the window, far above reality (761 in 90d)
    } while (after);
 ...
 * Returns:
 * {
 *   views:   { d3, d7, d30, d90 }, Map<pid, count>
 *   atc:     { d3, d7, d30, d90 },
 *   orders:  { d3, d7, d30, d90 },
 *   revenue: { d3, d7, d30, d90 },
...
async function getMetricMaps(fastify, options = {}) {
  ...
  // The window list is PASSED to GA4 rather than imported there: recoSignals
  // requires ga4, so ga4 importing WINDOWS back would be a cycle.
  const ga4 = skuIndexPending ? null : await getGa4Windows(skuIndex, WINDOWS);
  ...
  // Both shapes are keyed off WINDOWS so a new window needs no edit here: the
  // GA4 maps hold {views, atc} per product, the first-party ones are already
  // one map per window in WINDOWS order.
  const pick = (winMaps, key) => Object.fromEntries(WINDOWS.map((d) => {
    const src = winMaps['d' + d] || new Map();
    return ['d' + d, new Map([...src].map(([pid, m]) => [pid, m[key]]).filter(([, v]) => v > 0))];
  }));
  const byWindow = (maps) => Object.fromEntries(WINDOWS.map((d, i) => ['d' + d, maps[i] || new Map()]));

  const views = ga4 ? pick(ga4, 'views') : byWindow(beaconViews);
  const atc = ga4 ? pick(ga4, 'atc') : byWindow(cartAtc);
