...
// ---------------------------------------------------------------------------
// Every GA4 call is on a deadline.
//
// This is not defensive dressing — it is the fix for a real outage. On 22 Sep
// 2026 the store-wide pass reached 130 of 1,055 collections in seven minutes
// and then stopped dead for fifteen hours: no error, no heartbeat, nothing in
// the run record. Every SHOPIFY call is already bounded (fetchWithRetry aborts
// at 60s), but these four GA4 calls used a raw fetch() with no signal, so a
// single stalled request never settled. getServerCache dedupes on the in-flight
// promise, which turns one hung request into every later caller waiting on it
// forever — which is also why, on an earlier occasion, four separate rules all
// unblocked within the same second.
//
// A timeout turns that into a caught error, and the callers already fall back:
// getGa4Windows drops to first-party signals, and getMetricMaps returns empty
// maps rather than hanging.
// ---------------------------------------------------------------------------
const TOKEN_TIMEOUT_MS = 15000;
const REPORT_TIMEOUT_MS = 60000; // the 90-day item report is genuinely large

async function ga4Fetch(url, options, timeoutMs = REPORT_TIMEOUT_MS) {
  ...
  try {
    ...
    if (err && err.name === 'AbortError') {
      throw new Error(`GA4 request timed out after ${timeoutMs}ms: ${String(url).slice(0, 80)}`);
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
async function getAccessToken() {
  return getServerCache('ga4:access-token', async () => {
    ...
    const res = await ga4Fetch(TOKEN_URL, {
      method: 'POST',
...
async function fetchByEvent(days, eventName, metricName) {
  ...
  let offset = 0;
  const limit = 10000;
  // 10 pages x 10k rows. The 90-day view_item report already returns ~52k item
  // rows on this property, which fills 6 — and hitting the cap does not error,
  // it just drops the least-viewed tail. Four spare pages is cheap insurance
  // against a growing catalogue losing items with no signal anywhere.
  for (let page = 0; page < 10; page++) {
    const res = await ga4Fetch(`${DATA_API}/properties/${propertyId}:runReport`, {
      method: 'POST',
...
// `windows` comes from the caller (lib/recoSignals.js owns the list) rather
// than being imported — recoSignals requires this module, so importing it back
// would be a cycle. The default keeps this callable on its own.
async function getGa4Windows(skuIndex, windows = [3, 7, 30, 90]) {
  if (!isGa4Configured()) return null;
  try {
    const entries = await Promise.all(windows.map((d) =>
      getServerCache(`ga4:items:${d}d`, () => fetchItemMetrics(d, skuIndex), { ttlMs: 60 * 60 * 1000 })
        .then((map) => ['d' + d, map])
    ));
...
// ---------------------------------------------------------------------------
// PER-DAY item behaviour — the Smart Collections performance view.
//
// Same eventName-pinned reports as fetchByEvent, with `date` as a second
// dimension, so each row is (day, item, count) and history can be rebuilt for
// any trailing window instead of only the rolling 3/7/30d totals. The `date`
// dimension is in the GA property's own timezone (IST for this property),
// which matches the IST day-bucketing used everywhere else in the backend.
// ---------------------------------------------------------------------------
async function fetchDailyByEvent(days, eventName, metricName) {
  const propertyId = process.env.GA4_PROPERTY_ID;
  const token = await getAccessToken();
  ...
  let offset = 0;
  const limit = 10000;
  for (let page = 0; page < 12; page++) { // days x items rows; 120k cap
    const res = await ga4Fetch(`${DATA_API}/properties/${propertyId}:runReport`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(`GA4 daily runReport failed: ${JSON.stringify(data.error || data).slice(0, 200)}`);
    for (const row of data.rows || []) rows.push(row);
    const total = Number(data.rowCount) || 0;
    offset += limit;
    if (offset >= total) break;
...
/**
 * Returns { views, atc }, each Map<'YYYY-MM-DD', Map<numericProductId, n>>,
 * for the trailing `days` window. Null when GA4 is not configured or the
 * report fails — callers fall back or record zeros, never crash.
 */
async function getGa4DailyMaps(skuIndex, days) {
  if (!isGa4Configured()) return null;
  try {
    // The cache key carries whether the SKU index was available: without it
    // the dominant item_id format (variant SKUs) is dropped and the result is
    // near-empty — caching that under the same key would starve every later
    // call for the TTL even after the index finishes building.
    return await getServerCache('ga4:daily:' + days + (skuIndex ? ':sku' : ':nosku'), async () => {
      const [viewRows, atcRows] = await Promise.all([
        fetchDailyByEvent(days, 'view_item', 'itemsViewed'),
        fetchDailyByEvent(days, 'add_to_cart', 'itemsAddedToCart')
      ]);
      const fold = (rows) => {
        const byDate = new Map();
        for (const row of rows) {
          const raw = row.dimensionValues?.[0]?.value || ''; // '20260903'
          const date = raw.length === 8 ? raw.slice(0, 4) + '-' + raw.slice(4, 6) + '-' + raw.slice(6, 8) : null;
          const nid = ga4ItemIdToProductId(row.dimensionValues?.[1]?.value || '', skuIndex);
          const n = Number(row.metricValues?.[0]?.value) || 0;
          if (!date || !nid || !n) continue;
          if (!byDate.has(date)) byDate.set(date, new Map());
          const m = byDate.get(date);
          m.set(nid, (m.get(nid) || 0) + n);
        }
        return byDate;
      };
      return { views: fold(viewRows), atc: fold(atcRows) };
    }, { ttlMs: 6 * 60 * 60 * 1000 });
  } catch (err) {
    console.error('[GA4] Daily maps unavailable:', err.message);
    return null;
  }
}
...
module.exports = { isGa4Configured, getGa4Windows, getGa4DailyMaps, ga4RunReport, ga4ItemIdToProductId };
