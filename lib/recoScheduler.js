...
// Clock maths, cadence vocabulary and the due-check are shared with
// lib/smartSortScheduler.js; only the mutable tick state below is local.
const {
  istParts, isRuleDue, syncModeOf, reapAbandonedRuns
} = require('./syncCadence');
...
async function tick(fastify) {
  try {
    ...
    for (const rule of due) {
      ...
      console.log(`[RecoScheduler] ${hhmm} IST — running rule "${rule.collectionHandle}" (${syncModeOf(rule)})`);
...
async function startRecoScheduler(fastify) {
  ...
  await reapAbandonedRuns(fastify.mongo.db, 'reco_runs', STALE_RUNNING_MS, 'RecoScheduler');
