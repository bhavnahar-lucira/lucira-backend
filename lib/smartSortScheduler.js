...
const { runSmartRule, runningSmartRules } = require('./smartCollections');
...
async function tick(fastify) {
  try {
    ...
    for (const rule of due) {
      ...
      // Detached: a long sync must never stall the tick loop.
      runSmartRule(fastify, rule, 'schedule').catch((err) => {
