 ...
 *                         plus the sync cadence (scheduleTime / syncMode /
 *                         syncWeekday) so changing "weekly instead of daily"
 *                         goes live with the ordering change it belongs to.
...
const { runSmartRule, runGlobalRule, isGlobalRule } = require('./smartCollections');

/**
 * Sync a rule with the runner that fits it.
 *
 * The global rule has `collectionId: null`, so handing it to runSmartRule ends
 * in `fetchLiveOrder(null)` and Shopify replying "Variable $id of type ID! was
 * provided invalid value" — the run is marked failed within a second and
 * NOTHING is synced. That is not hypothetical: it happened on 19 Sep 2026 at
 * 13:34 when a draft was published on the global rule. The scheduler and the
 * /run route both branch on isGlobalRule; publishing a draft and applying a
 * scheduled revert did not, which is the whole reason this helper exists
 * rather than the branch being written out twice more.
 */
const syncRule = (fastify, rule, trigger) =>
  (isGlobalRule(rule) ? runGlobalRule : runSmartRule)(fastify, rule, trigger);
...
/**
 * The configuration the ADMIN is working on: the draft's fields where it has
 * them, the live ones otherwise.
 *
 * The overlay is deliberately identical to publishDraft's, which gives the
 * property that matters — a preview built from this shows EXACTLY what
 * publishing would produce, field for field.
 *
 * Every admin-facing read goes through this: the editor's rail, the curate
 * modal and the curation save. Before, the editor read the draft while the
 * curate modal read live, so one rule could show two different orders at the
 * same time with nothing on screen to say why. That looked like an engine bug
 * and was not one. The engine itself never calls this — a SYNC must always
 * push what is live, never a draft.
 */
function effectiveRule(rule) {
  if (!rule || !rule.draft) return rule;
  const out = { ...rule };
...
/**
 * What publishing this rule's draft would DESTROY: curation that is live now
 * and absent from the draft.
 *
 * This is not hypothetical. A draft saved on 19 Sep held `positions: []` while
 * seven hand-placements were added live afterwards from the curate modal;
 * publishing would have deleted all seven without a word, because publishDraft
 * copies every ORDER_FIELD across. Curating now writes to the draft when one
 * exists, so new drafts do not drift like that — but drafts made before then
 * still can, and a merchant should never lose hand-placed work silently.
 */
function curationLoss(rule) {
  const d = (rule && rule.draft) || null;
  if (!d) return { lostPins: [], lostRemoved: [], lostPositions: [], any: false };
  const fieldOr = (f) => (d[f] !== undefined ? d[f] : (rule[f] || []));
  const ids = (list) => new Set((list || []).map(String));

  const draftPins = ids(fieldOr('pinned'));
  const draftRemoved = ids(fieldOr('removed'));
  const draftPositions = new Set((fieldOr('positions') || []).map((e) => String(e && e.id)));

  const lostPins = (rule.pinned || []).map(String).filter((g) => !draftPins.has(g));
  const lostRemoved = (rule.removed || []).map(String).filter((g) => !draftRemoved.has(g));
  const lostPositions = (rule.positions || []).filter((e) => !draftPositions.has(String(e && e.id)));

  return {
    lostPins,
    lostRemoved,
    lostPositions,
    any: Boolean(lostPins.length || lostRemoved.length || lostPositions.length)
  };
}
...
async function publishDraft(fastify, rule, { trigger = 'manual', sync = true } = {}) {
  ...
  if (draft.syncMode) $set.syncMode = draft.syncMode;
  if (draft.syncWeekday !== undefined) $set.syncWeekday = draft.syncWeekday;
  ...
  if (sync) {
    ...
    syncRule(fastify, updated, trigger === 'schedule' ? 'schedule' : 'manual').catch((err) => {
...
async function applyRevert(fastify, rule) {
  ...
  syncRule(fastify, updated, 'schedule').catch((err) => {
...
module.exports = {
  ORDER_FIELDS, configOf, writeVersion, publishDraft, applyRevert,
  effectiveRule, curationLoss
};
