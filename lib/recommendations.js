...
const { loadCardPriceResolver } = require('./cardPrice');
...
const PRODUCT_NODE_FIELDS = `
  ...
  variants(first: 5) { nodes { availableForSale title position price compareAtPrice selectedOptions { name value } } }
  stoneType: metafield(namespace: "custom", key: "stone_type") { value }
  materialType: metafield(namespace: "ornaverse", key: "material_type") { value }
...
const mapNode = (node) => {
  ...
  return {
    ...
    // The first few variants in position order. The card price falls back to
    // the first one when nothing is in stock (see lib/cardPrice.js).
    leadVariants: (node.variants?.nodes || []).map((v) => ({
      title: v.title || '',
    ...
    // ornaverse.material_type — a single_line_text_field choice list
    // ("Gold & Diamond", "Platinum & Diamond", "Gold,Diamond & Gemstone"), so
    // it needs none of the JSON unwrapping carat_range does.
    materialType: node.materialType?.value || null,
...
// ---------------------------------------------------------------------------
// Choice lists read from the metafield DEFINITION rather than from the data.
//
// Deriving a dropdown from the catalogue scan can only ever offer values some
// product already carries. Measured on this shop 22 Sep 2026: material_type
// showed 5 options that way, while its definition allows 10. The five missing
// ones are exactly the ones a merchant reaches for when writing a rule ahead
// of the products — "Silver & Diamond" before any silver piece is live — and
// their absence reads as "that value does not exist" rather than "nothing uses
// it yet".
//
// Only definitions that actually declare `choices` are listed here. On this
// shop stone_type does (CZ / Labgrown / Natural / Fancy Gemstone) and
// shop_for / carat_range do not, so those keep falling back to the scan.
// ---------------------------------------------------------------------------
const CHOICE_METAFIELDS = {
  material_type: { namespace: 'ornaverse', key: 'material_type' },
  stone_type: { namespace: 'custom', key: 'stone_type' }
};

const METAFIELD_CHOICES_QUERY = `
  query recoMetafieldChoices($namespace: String!, $key: String!) {
    metafieldDefinitions(first: 1, ownerType: PRODUCT, namespace: $namespace, key: $key) {
      nodes { name validations { name value } }
    }
  }
`;

async function getMetafieldChoices() {
  return getServerCache('reco:metafield-choices', async () => {
    const out = {};
    for (const [attr, { namespace, key }] of Object.entries(CHOICE_METAFIELDS)) {
      try {
        const data = await shopifyAdminFetch(METAFIELD_CHOICES_QUERY, { namespace, key });
        const node = data?.metafieldDefinitions?.nodes?.[0];
        const raw = (node?.validations || []).find((v) => v.name === 'choices')?.value;
        if (!raw) continue;
        // `choices` arrives as a JSON array string, e.g. '["Gold & Diamond",...]'.
        const list = JSON.parse(raw);
        if (Array.isArray(list) && list.length) out[attr] = cleanFacet(list);
      } catch (err) {
        console.warn(`[Reco] Metafield choices for ${namespace}.${key} unavailable:`, err.message);
      }
    }
    const summary = Object.entries(out).map(([k, v]) => `${k}:${v.length}`).join(' ');
    if (summary) console.log(`[Reco] Metafield choice lists loaded — ${summary}`);
    return out;
  }, { ttlMs: 60 * 60 * 1000 });
}
...
async function getAttributeOptions() {
  ...
  if (_lastCatalogScan) {
    ...
    options.material_type = cleanFacet(_lastCatalogScan.map((x) => x.materialType));
  ...
  // The DEFINITION wins wherever it declares choices: it is the complete list,
  // while the scan is only what happens to be in use today. Awaited last so a
  // slow metafield read cannot hold up the facets that are already in hand.
  Object.assign(options, await getMetafieldChoices().catch(() => ({})));
...
const ATTRIBUTES = {
  // Product details
  // `price` is the LOWEST variant price (any size/purity, stocked or not) —
  // not what the card shows. card_price is the price shoppers actually see.
  price:            { label: 'Lowest variant price', group: 'Product details', kind: 'number', matchable: true, get: (p) => p.price },
  card_price:       { label: 'Card price (in-stock variant)', group: 'Product details', kind: 'number', matchable: true, cardPrice: 'price' },
  card_discount_percent: { label: 'Card discount % (in-stock variant)', group: 'Product details', kind: 'number', matchable: true, cardPrice: 'discount' },
  ...
  material_type:    { label: 'Material type', group: 'Product details', kind: 'string', matchable: true, get: (p) => p.materialType },
  ...
  // Performance — 3D / 7D / 30D / 90D windows.
  //
  // 90 days exists because 30 leaves too much of a 3,000-product catalogue with
  // no signal to rank on, so slots fall back to scan order. Measured 22 Sep
  // 2026: add-to-cart coverage 3,155 items (30d) -> 5,904 (90d), views 23,440
  // -> 52,397. Views/carts come from GA4, which holds the history; orders and
  // revenue from Shopify, which holds all of it.
  ...
  views_90d:   { label: 'Product views (last 90 days)', group: 'Performance', kind: 'number', matchable: false, metric: ['views', 'd90'] },
  ...
  atc_90d:     { label: 'Add to carts (last 90 days)', group: 'Performance', kind: 'number', matchable: false, metric: ['atc', 'd90'] },
  ...
  orders_90d:  { label: 'Orders (last 90 days)', group: 'Performance', kind: 'number', matchable: false, metric: ['orders', 'd90'] },
  ...
  revenue_90d: { label: 'Revenue (last 90 days)', group: 'Performance', kind: 'number', matchable: false, metric: ['revenue', 'd90'] },
  ...
  atc_rate_90d: { label: 'Add to cart rate % (last 90 days)', group: 'Performance', kind: 'number', matchable: false, ratio: ['atc', 'views', 'd90'] },
...
const SORT_KEYS = {
  ...
  views_90d:        { label: 'Product views (90 days)', directional: true },
  ...
  atc_90d:          { label: 'Add to carts (90 days)', directional: true },
  ...
  atc_rate_90d:     { label: 'Add to cart rate (90 days)', directional: true },
  ...
  orders_90d:       { label: 'Orders (90 days)', directional: true },
  ...
  revenue_90d:      { label: 'Revenue (90 days)', directional: true },
  ...
  price:            { label: 'Lowest variant price', directional: true },
  card_price:       { label: 'Card price (in-stock variant)', directional: true },
  card_discount_percent: { label: 'Card discount % (in-stock variant)', directional: true }
};
...
function attrValue(key, product, ctx) {
  ...
  if (def.cardPrice) {
    // The variant the storefront card prices. A missing resolver means the
    // in-stock scan failed for a rule that does NOT use card prices (a rule
    // that does fails in loadCardPriceResolver), so fall back quietly.
    const card = ctx.cardPriceOf ? ctx.cardPriceOf(product) : null;
    if (def.cardPrice === 'discount') {
      if (!card) return product.discountPercent || 0;
      return card.compareAtPrice > card.price ? ((card.compareAtPrice - card.price) / card.compareAtPrice) * 100 : 0;
    }
    return card ? card.price : product.price;
  }
  if (def.ratio) {
...
async function buildContext(fastify, norm, opts = {}) {
  ...
  const [collectionProducts, catalogProducts, popMaps, metrics, cardPriceOf] = await Promise.all([
