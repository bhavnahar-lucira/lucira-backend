...
// Admin GraphQL cost model: each product node below costs ~10-11 points
// (product + image + price ranges + variants + 5 metafields). The single-query
// cost ceiling is 1,000, so we page at 80 (~880 points) and let fetchWithRetry
// absorb 429 throttling between pages.
const SCAN_PAGE_SIZE = 80;
const SCAN_MAX_PAGES = 100;           // ~8k product cap per scan
...
async function pagedScan(query, baseVars, unwrap, label) {
  ...
  let pages = 0;
  do {
    const data = await shopifyAdminFetch(query, { ...baseVars, first: SCAN_PAGE_SIZE, after });
...
function normalizeRule(rule) {
  ...
  return {
    source: { collectionId: rule.collectionId || null, conditions: [] },
    sequences,
    commonConditions: [],
...
async function computeForRule(fastify, rule, options = {}) {
  ...
  const eligible = ctx.collectionPool.filter((p) => evalConditions(p, null, norm.source.conditions, ctx));
...
module.exports = {
  attrValue,          // exported for unit tests of the condition maths
  computeForRule,
  previewForRule,
  previewScope,
  runRule,
