      ...
      do {
        ...
        for (const node of page.nodes || []) {
          ...
          products.push({
            ...
            // Buyable, NOT "has stock": this is a made-to-order catalogue where many
            // variants sit at inventoryQuantity 0 with inventoryPolicy CONTINUE and
            // availableForSale true. Gating on totalInventory alone would wrongly
            // exclude products a shopper can buy right now.
...
//   inventory    = buyable (availableForSale / untracked / in stock) ? 1 : 0
...
const computeSlotsForProduct = (source, pool, rule, popByGid, maxPop) => {
  ...
  blocks.forEach((block, blockIndex) => {
    const size = Math.max(0, Math.min(Number(block.size) || 0, MAX_RECOMMENDATIONS - picked.length));
    ...
    let survivors = pool.filter((cand) => {
      ...
      if (conditions.inStock && !cand.buyable) return false;
...
async function previewForRule(fastify, rule, { productId, limit } = {}) {
  ...
  // Apply the same ownership exclusion the run uses, or preview would show slots
  // for products a higher-priority rule owns and the run will skip.
...
async function runRule(fastify, rule, trigger, opts = {}) {
  ...
  // opts.preAcquired: the caller (POST /rules/:id/run) already claimed the key
  // synchronously so it could reply with a runId that is guaranteed to be real.
  // The finally block still releases it either way.
  if (!opts.preAcquired) {
  ...
  try {
    ...
    try {
      ...
      for (const r of results) {
        ...
        // An empty list is rejected by Shopify ("Value can't be blank") and, because
        // metafieldsSet is atomic per batch, one empty entry would fail up to 24 good
        // writes alongside it. Leave the existing value untouched instead.
      ...
      for (let i = 0; i < toWrite.length; i += WRITE_BATCH_SIZE) {
        ...
        try {
          ...
          if (userErrors.length) {
            ...
            // metafieldsSet is ATOMIC per call: if any input errors, none of the batch is
            // written. Counting the rest as "written" would make lastRunStats lie and mask
            // a recurring bad input. The whole batch failed.
            stats.failed += batch.length;
          } else {
            stats.written += batch.length;
            // Keep the cached scan in step with what we just wrote; otherwise a second
            // run inside the scan TTL sees stale currentValue and rewrites every product.
